...
export default function VendorsPage() {
  ...
  const vendorItems = useMemo(
    () => items.filter((i) => i.vendor_id === selectedVendorId),
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Manage vendors and view their catalog
            </p>
          </div>
          <Button onClick={openAddVendor} icon={Plus}>
            Add Vendor
          </Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Vendors list skeleton */}
          <div className="bg-white rounded-lg shadow border">
            ...
            <ul className="divide-y">
              {Array.from({ length: 6 }).map((_, i) => (
                <li key={`vendor-skel-${i}`} className="px-4 py-3">
          ...
          <div className="md:col-span-2 bg-white rounded-lg shadow border min-h-[360px] p-6">
            <div className="mb-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <div
                    key={`vendor-items-skel-${i}`}
                    className="grid grid-cols-12 gap-4"
                  >
                    <Skeleton className="h-5 col-span-6" />
        ...
        <div className="bg-white rounded-lg shadow border">
          <div className="px-4 py-3 border-b">
            <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <Store className="h-5 w-5" /> All Vendors ({vendors.length})
            </h2>
          </div>
          <ul className="divide-y">
            {vendors.map((v) => (
              <li key={v.id}>
                <button
                  className={`w-full text-left px-4 py-3 hover:bg-gray-50 ${
                    selectedVendorId === v.id ? "bg-blue-50" : ""
                  }`}
          ...
          {!selectedVendor ? (
            <div className="h-full p-8 flex items-center justify-center text-gray-500">
              <div className="flex items-center gap-2">
                <PackageSearch className="h-5 w-5" /> Select a vendor to view
                details
              </div>
            </div>
          ) : (
            <div className="p-6">
              <div className="mb-4">
                ...
                <div className="mt-2 space-y-1 text-sm text-gray-600">
                  {selectedVendor.contact_info && (
                    <div className="flex items-center gap-2">
                      <User className="h-4 w-4" /> {selectedVendor.contact_info}
                    </div>
                  )}
                  {selectedVendor.address && (
                    <div className="flex items-center gap-2">
                      <MapPin className="h-4 w-4" /> {selectedVendor.address}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">
                  Items from this vendor ({vendorItems.length})
                </h3>
                  ...
                  <Table>
                    <THead className="bg-gray-50">
                      <TR>
                        ...
                        <TH className="px-3 py-2">Current Vendor</TH>
                      </TR>
                    </THead>
