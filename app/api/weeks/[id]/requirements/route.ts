...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, week_plan_id, item_id, required_qty, to_buy_override, items(name, unit, vendor_id, vendors(name))")
    .eq("week_plan_id", id)
  ...
  const normalized = (data ?? []).map((r) => ({
    ...
    item: r.items
      ? {
          id: r.item_id,
          name: (r.items as any).name as string,
          unit: (r.items as any).unit as string,
          vendor_id: (r.items as any).vendor_id as number | null,
          vendor_name: ((r.items as any).vendors as any)?.name ?? null,
